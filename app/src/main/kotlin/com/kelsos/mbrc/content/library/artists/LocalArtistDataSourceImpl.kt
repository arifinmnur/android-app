...
@Inject constructor(private val dispatchers: AppDispatchers) : LocalArtistDataSource {
  ...
  override suspend fun loadAllCursor(): List<Artist> = withContext(dispatchers.db) {
  ...
  override suspend fun search(term: String): List<Artist> = withContext(dispatchers.db) {
    val query = (select from Artist::class where Artist_Table.artist.like("%${term.escapeLike()}%"))
    return@withContext query.flowQueryList()
  }

  override suspend fun getAlbumArtists(): List<Artist> = withContext(dispatchers.db) {
    val query = SQLite.select().distinct()
      .from(Artist::class.java)
      .innerJoin(Track::class.java)
